describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let itemShadowRoot = await prodItem[0].getProperty("shadowRoot");
    var button = await itemShadowRoot.$('button');
    await button.click();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let itemShadowRoot = await prodItems[i].getProperty("shadowRoot");
      let button = await itemShadowRoot.$("button");
      let buttonInnerText = await button.getProperty("innerHTML");
      expect(buttonInnerText["_remoteObject"].value).toBe("Remove from Cart");
    }
    var cartCount = await page.$("#cart-count");
    cartCount = await cartCount.getProperty("innerHTML");
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let button = await itemShadowRoot.$("button");
      let buttonInnerText = await button.getProperty("innerHTML");
      expect(buttonInnerText["_remoteObject"].value).toBe("Add to Cart");
    }
    var cartCount = await page.$("#cart-count");
    cartCount = await cartCount.getProperty("innerHTML");
    expect(cartCount["_remoteObject"].value).toBe("0");
